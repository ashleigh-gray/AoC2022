...
for step in moves:
    ...
    for c in range(current_move.number):
        df.at[heights[current_move.end].height+1, current_move.end] = df.at[heights[current_move.start].height, current_move.start]
        df.at[heights[current_move.start].height, current_move.start] = np.NaN
